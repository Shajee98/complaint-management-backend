...
export const createComplaint: RequestHandler = async (req, res, next) => {
  try {
    ...
    const { customer_number, complaint_number, description, staff_id, department_id, complaint_status_id, complaint_type_id, fromWhatsapp, customer_name, created_by } = req.body
    ...
    const complaint = await complaintService.createComplaint({customer_number, complaint_number, description, staff_id, department_id, complaint_status_id, complaint_type_id, fromWhatsapp, customer_name, created_by})
    if (!complaint) {
...
export const getAllComplaintswoUser: RequestHandler = async (req, res, next) => {
  try {
    const complaints = await complaintService.getAllComplaintsWoUsers()
    if (!complaints) {
      return serverErrorResponse(res, responses.ORDER_CREATED);
    }
